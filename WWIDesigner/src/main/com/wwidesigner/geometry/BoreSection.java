...
class BoreSection implements ComponentInterface
{
			...
			PhysicalParameters params)
	{
		...
		if (mLeftRadius == mRightRadius) // the case of a cylindrical segment
		{
			...
			return new TransferMatrix(coshL, sinhL.multiply(ZcLeft), sinhL.divide(ZcLeft), coshL);
		}

		// the case of a cone
		double ZcRight = params.calcZ0(mRightRadius);

		double one_over_x_in = (mRightRadius-mLeftRadius) / (mLeftRadius*mLength);
		double one_over_x_out = (mRightRadius-mLeftRadius) / (mRightRadius*mLength);

		// inverse of the equivalent radius at which we calculate the losses
		double one_over_Req = Math.log(mRightRadius/mLeftRadius) / (mRightRadius - mLeftRadius);

		Complex k_lossy = Complex.valueOf(1,-1).multiply(one_over_Req*Math.sqrt(wave_number)*params.getAlphaConstant()).add(wave_number);		

		Complex k_lossy_L = k_lossy.multiply(mLength);

		Complex A = k_lossy_L.cos().multiply(mLeftRadius/mLeftRadius).subtract( k_lossy_L.sin().multiply(one_over_x_in).divide(k_lossy) );
		Complex B = k_lossy_L.sin().multiply(Complex.I).multiply((mRightRadius/mLeftRadius)*ZcRight);
		Complex C = Complex.valueOf(mRightRadius/mLeftRadius).multiply(Complex.I.multiply(k_lossy_L.sin()).multiply(k_lossy.pow(-2).multiply(one_over_x_in*one_over_x_out).add(1))
				.add( k_lossy_L.cos().multiply(one_over_x_in-one_over_x_out).divide(Complex.I.multiply(k_lossy)))).divide(ZcLeft);
		Complex D = k_lossy_L.cos().multiply(mLeftRadius/mLeftRadius).add( k_lossy_L.sin().multiply(one_over_x_out).divide(k_lossy) );
