...
public class SketchInstrument
{
	...
	protected void drawMouthpiece(Chart chart)
	{
		...
		if (window != null)
		{
			...
			modelWindow.addPoint(mouthpiece.getBorePosition(),
					-0.5 * window.getWindowWidth());
			modelWindow.addPoint(
					mouthpiece.getBorePosition() - window.getWindowLength(),
					-0.5 * window.getWindowWidth());
			modelWindow.addPoint(
					mouthpiece.getBorePosition() - window.getWindowLength(),
					0.5 * window.getWindowWidth());
			...
			if (window.getWindwayLength() != null)
			{
				...
				ChartStyle styleWindway = new ChartStyle(Color.black, false,
						true);
				DefaultChartModel modelWindway = new DefaultChartModel(
						"Windway");
				float[] dashes = new float[2];
				dashes[0] = 5.0f;
				dashes[1] = 2.0f;
				...
				double windwayExit = mouthpiece.getBorePosition()
						- window.getWindowLength();
	...
	public void draw(Instrument instrument, final String instrumentFileName,
			final boolean exitOnClose)
	{
		...
		SwingUtilities.invokeLater(new Runnable()
		{
			public void run()
			{
				String title = getTitleName(instrumentFileName, mName);
				...
				frame.setSize(PIXELS_ACROSS, 110 + (int) ((PIXELS_ACROSS - 110)
						* boreDimensions[1] / boreDimensions[0]));
	...
	protected String getTitleName(String instrumentFileName,
			String instrumentName)
	{
		StringBuffer buf = new StringBuffer();
		buf.append("Sketch of ");
		if (instrumentFileName != null
				&& instrumentFileName.trim().length() > 0)
		{
			buf.append(instrumentFileName.trim() + ": ");
		}
