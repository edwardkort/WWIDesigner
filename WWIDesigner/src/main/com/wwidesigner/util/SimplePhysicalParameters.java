...
public class SimplePhysicalParameters
{
	...
	public SimplePhysicalParameters(double temperature, TemperatureType tempType)
	{
		...
		mEta = 3.648e-6d * (1d + 0.0135003d * (mTemperature + 273.15d));

		double deltaT = mTemperature - 26.85d;

		mRho = 1.1769d * (1d - 0.00335d * deltaT);
		mMu = 1.8460E-5d * (1d + 0.00250d * deltaT);
		mGamma = 1.4017d * (1d - 0.00002d * deltaT);
		mNu = 0.8410d * (1d - 0.00020d * deltaT);
		mWaveNumber1 = 2.0d * Math.PI / mSpeedOfSound;

		mAlphaConstant = Math.sqrt(mMu / (2d * mRho * mSpeedOfSound))
				* (1d + (mGamma - 1d) / mNu);
