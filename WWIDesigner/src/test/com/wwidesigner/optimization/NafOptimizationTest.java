...
public class NafOptimizationTest extends AbstractOptimizationTest
{
	...
	public final void test7HoleTaperOptimization()
	{
		...
		{
			setInputInstrumentXML(
					"com/wwidesigner/optimization/example/G7HoleNAF.xml");
			setInputTuningXML(
					"com/wwidesigner/optimization/example/G7HoleNAFTuning.xml");
			setParams(new PhysicalParameters(22.22, TemperatureType.C));
			setCalculator(new NAFCalculator());
			setup();
			...
			setUpperBound(new double[] { 0.5, 0.15, 0.18, 0.21, 0.24, 0.27, 0.3,
					0.3, 0.012, 0.012, 0.012, 0.012, 0.012, 0.012, 0.012 });
			evaluator = new ReactanceEvaluator(calculator);
			...
			PositionInterface[] sortedPoints = Instrument.sortList(borePoints);
			PositionInterface lastPoint = sortedPoints[sortedPoints.length - 1];
			// Accommodate varying hole-size multiplier
	...
	public final void test7HoleTaperOptimization2()
	{
		try
		{
			...
			setCalculator(new NAFCalculator());
			setup();
			setLowerBound(new double[] { 0.2, 0.014, 0.014, 0.014, 0.014, 0.014,
					0.0005, 0.05, 0.0025, 0.0025, 0.0025, 0.0025, 0.0025,
					0.0013, 0.0013 });
			setUpperBound(
					new double[] { 0.5, 0.05, 0.05, 0.1, 0.05, 0.05, 0.003, 0.2,
	...
	public final void test1HoleOptimization()
	{
		...
		{
			setInputInstrumentXML(
					"com/wwidesigner/optimization/example/1HoleNAF1.xml");
			setInputTuningXML(
			...
			assertEquals("Hole 1 diameter incorrect", 0.39,
					sortedHoles.get(0).getDiameter(), 0.02); // 0.398

			// This hole position derives from the actual instrument AND 2 other
			// calculation algorithms.
			assertEquals("Hole 1 position incorrect", 7.5,
					sortedHoles.get(0).getBorePosition(), 0.1); // 8.1
	...
	public final void test6HoleOptimization()
	{
		...
		{
			setInputInstrumentXML(
					"com/wwidesigner/optimization/example/6HoleNAF1.xml");
			setInputTuningXML(
					"com/wwidesigner/optimization/example/6HoleNAF1Tuning.xml");
			setParams(new PhysicalParameters(22.22, TemperatureType.C));
			setCalculator(new NAFCalculator());
			setup();
			setLowerBound(new double[] { 0.1905, 0.25, 0.02032, 0.02032,
					0.02032, 0.02032, 0.02032, 0.002032, 0.003175, 0.003175,
					0.003175, 0.003175, 0.003175 });
			setUpperBound(new double[] { 0.6985, 0.5, 0.03175, 0.03175, 0.0762,
					0.03175, 0.03175, 0.0127, 0.0127, 0.0127, 0.0127, 0.0127,
					0.0127 });
			evaluator = new ReactanceEvaluator(calculator);
			objective = new HoleFromTopObjectiveFunction(calculator, tuning,
					evaluator, BoreLengthAdjustmentType.PRESERVE_TAPER);
			...
			assertEquals("Hole 1 diameter incorrect", 0.340,
					sortedHoles.get(0).getDiameter(), 0.1); // 0.308
			assertEquals("Hole 2 diameter incorrect", 0.425,
					sortedHoles.get(1).getDiameter(), 0.1); // 0.361
			assertEquals("Hole 3 diameter incorrect", 0.416,
					sortedHoles.get(2).getDiameter(), 0.1); // 0.352
			assertEquals("Hole 4 diameter incorrect", 0.390,
					sortedHoles.get(3).getDiameter(), 0.1); // 0.357
			assertEquals("Hole 5 diameter incorrect", 0.407,
					sortedHoles.get(4).getDiameter(), 0.1); // 0.379
			assertEquals("Hole 6 diameter incorrect", 0.375,
					sortedHoles.get(5).getDiameter(), 0.1); // 0.398

			assertEquals("Hole 1 position incorrect", 2.915,
					sortedHoles.get(0).getBorePosition(), 0.5); // 3.15
			assertEquals("Hole 2 position incorrect", 3.715,
					sortedHoles.get(1).getBorePosition(), 0.5); // 4.0
			assertEquals("Hole 3 position incorrect", 4.515,
					sortedHoles.get(2).getBorePosition(), 0.5); // 4.85
			assertEquals("Hole 4 position incorrect", 5.315,
					sortedHoles.get(3).getBorePosition(), 0.5); // 5.9
			assertEquals("Hole 5 position incorrect", 6.322,
					sortedHoles.get(4).getBorePosition(), 0.5); // 7.0
			assertEquals("Hole 6 position incorrect", 7.122,
					sortedHoles.get(5).getBorePosition(), 0.5); // 8.1
